...
from Functions.utils import target_distribution
...
def train(
    ...
    stopping_delta: Optional[float] = float,
    collate_fn=default_collate,
    cuda: bool = True,
    sampler: Optional[torch.utils.data.sampler.Sampler] = None,
    silent: bool = False,
    update_freq: int = 10,
    evaluate_batch_size: int = 10,
    ...
    kmeans = KMeans(n_clusters=model.cluster_number, n_init=50) # Lets change this to 40
    model.train()
    features = []
    for index, batch in enumerate(data_iterator):
        ...
        feature = model.encoder(batch)[0].detach().cpu()
        flat_feature = torch.flatten(feature, 1)
        features.append(flat_feature)

    y_predicted = kmeans.fit_predict(torch.cat(features).numpy())

    y_predicted_previous = torch.tensor(np.copy(y_predicted), dtype=torch.long)
    ...
    for epoch in range(epochs):
        ...
        for index, batch in enumerate(data_iterator):
            ...
            unflat_feature = model.encoder(batch)[0].detach().cpu()
            flatty_feature = torch.flatten(unflat_feature, 1)
            features.append(flatty_feature)
        ...
        y_predicted_delta, feature_predict = predict(
            dataset,
            model,
        ...
        print("y_predicted previous")
        print(y_predicted_previous)
        print("predicted delta")
        print(y_predicted_delta)

        delta_label = (
                float((y_predicted_delta != y_predicted_previous).float().sum().item())
                / y_predicted_previous.shape[0]
        )
        ...
        y_predicted_previous = y_predicted_delta
...
def predict(
    ...
    for batch in data_iterator:
        if (isinstance(batch, tuple) or isinstance(batch, list)) and len(batch) == 2:
            batch, _ = batch  # unpack if we have a prediction label
            if return_actual:
                raise ValueError(
        ...
        if cuda:
            batch = batch.cuda(non_blocking=True)
        feature = model(batch).detach().cpu()
        features.append(feature)

    if return_actual:
        features = torch.cat(features).max(1)[1]
        actual = torch.cat(actual).long()
        return features, actual
    else:
        feature_map = torch.cat(features).max(1)[1]
        feature_delta = torch.cat(features)
        print(feature_delta)
        print(feature_delta.shape)
        print(feature_map)
        return feature_map, feature_delta
